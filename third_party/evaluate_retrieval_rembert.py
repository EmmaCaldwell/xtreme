...
ALL_MODELS = set(
    transformers.BERT_PRETRAINED_MODEL_ARCHIVE_LIST + transformers.REMBERT_PRETRAINED_MODEL_ARCHIVE_LIST
)

MODEL_CLASSES = {
    "bert": (BertConfig, BertModel, BertTokenizer),
    "rembert": (RemBertConfig, RemBertModel, RemBertTokenizer),
...
def prepare_batch(sentences, tokenizer, model_type, device="cuda", max_length=512, lang='en', langid=None, use_local_max_length=True, pool_skip_special_token=False):
  ...
  if model_type == 'rembert':
    ...
    raise ValueError("Only RemBert is supported here.")
...
def main():
  ...
  if args.task_name == 'bucc2018':
    ...
    src_tok_file = os.path.join(args.output_dir,'{}-en.{}'.format(src_lang2, src_lang2))
    print("\n PATH \n")
